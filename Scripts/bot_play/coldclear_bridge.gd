...
## 逐步转为 BotAction 供 PieceController 消费。
...
## 踢墙表来自 piece_controller.kick_table["all"]（游戏 asc 踢墙表），空/缺失时 worker 用标准 SRS。
func _build_game_rules_command(game_controller) -> String:
	...
	# 读取游戏 asc 踢墙表（piece_controller.kick_table["all"]，元素为 [dx,dy]）
