...
func _auto_finding():
	if not piece_controller:
		piece_controller = get_node_or_null("../MinoBoardDrawer")
		if not piece_controller:
			push_error("PieceController: 未找到MinoBoardDrawer节点！")
			return
			
	if not garbage_line_controller:
		garbage_line_controller = get_node_or_null("../MinoBoardDrawer")
		if not garbage_line_controller:
			push_error("PieceController: 未找到MinoBoardDrawer节点！")
			return
	...
	if not board_drawer:
		...
		if not board_drawer:
			board_drawer = get_node_or_null("../../MainBoard/MinoBoardDrawer")
...
func _extra_data_deal():
	...
	# 【DH】双洞：每行垃圾行 50% 概率额外加一个洞（含全旋惩罚行）
	if extra_data_dict.has("doublehole"):
		garbage_line_controller.doublehole_enabled = bool(extra_data_dict["doublehole"])
	# 【MS】混乱：垃圾混乱度增加 + 易挖度降低
	if extra_data_dict.has("messy"):
		garbage_line_controller.messy_enabled = bool(extra_data_dict["messy"])
	
	if extra_data_dict.has("invisible"):
		if board_drawer:
			board_drawer.invisible = extra_data_dict["invisible"]
	...
	if extra_data_dict.has("allspin"):
		clear_line_controller.allspin = extra_data_dict["allspin"]
	
	# 【rAS】逆位全旋（邪术师 The Warlock）
	if extra_data_dict.has("rAS"):
		var ras_on: bool = bool(extra_data_dict["rAS"])
		clear_line_controller.rAS = ras_on
		if ras_on:
			# 垃圾行保护（QP2 受击权重机制）
			garbage_line_controller.hit_weight_protection = true
			# 注入开局 10 行垃圾（仅一次，每列一个洞）——延迟到所有节点就绪后执行
			garbage_line_controller.call_deferred("add_ras_initial_garbage", 10)
	...
	if extra_data_dict.has("NoHold") or extra_data_dict.has("no_hold"):
		...
		if piece_controller:
			piece_controller.no_hold = no_hold_value
		if board_drawer:
			board_drawer.no_hold = no_hold_value
	
	if extra_data_dict.has("BotMode"):
		piece_controller.bot_mode = true
	# 可调整的 Bot PPS（buff 界面 BotPlay 旁的输入框）
	if extra_data_dict.has("bot_target_pps"):
		piece_controller.bot_target_pps = float(extra_data_dict["bot_target_pps"])
...
func _process(delta: float) -> void:
	...
	piece_controller.gravity_drop_time = default_get_oneD_array_things(current_stage,gravity_drop_time_array)
	piece_controller.lock_delay = default_get_oneD_array_things(current_stage,lock_delay_array)
