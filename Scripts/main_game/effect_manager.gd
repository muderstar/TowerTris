...
# ---- DS Crystal 字体（rAS 逆位全旋蓄力数字专用，与全旋倒计时同源）----
...
## 【rAS】逆位全旋专用：先紫后红（紫→红长渐变，红在 138）
const B2B_COLOR_GRADIENT_RAS: Array = [
	[0, 4, Color(0.60, 0.20, 1.00), Color(0.70, 0.15, 1.00)],   # 紫（蓄满起点）
	[4, 60, Color(0.70, 0.15, 1.00), Color(1.00, 0.30, 0.70)],  # 紫→品红过渡
	[60, 138, Color(1.00, 0.30, 0.70), Color(1.00, 0.10, 0.05)], # 品红→红（长过渡，138 处纯红）
	[138, 999999, Color(1.00, 0.10, 0.05), Color(1.00, 0.10, 0.05)], # 138+ 保持红
]
## rAS 模式标志：true 时蓄力数字用 DS Crystal 字体 + rAS 红渐变
var _b2b_ras_mode: bool = false
...
## 设置 rAS 模式（true = 逆位全旋：DS Crystal 蓄力数字 + 红渐变）
func set_ras_mode(on: bool) -> void:
	_b2b_ras_mode = on

## 当前是否处于 rAS 模式
func is_ras_mode() -> bool:
	return _b2b_ras_mode

func _get_b2b_color_at(charge: int, charge_at: int) -> Color:
	if charge < charge_at:
		return B2B_DIM_COLOR
	var gradient: Array = B2B_COLOR_GRADIENT_RAS if _b2b_ras_mode else B2B_COLOR_GRADIENT
	for stop: Array in gradient:
	...
	return gradient[-1][3] as Color

## 懒加载 DS Crystal 字体（rAS 蓄力数字用；失败回退 fallback）
func _get_ras_font() -> FontFile:
	...
	if _ras_font.load_dynamic_font(SEG_FONT_PATH) != OK:
		push_warning("cristal.ttf 加载失败，rAS 蓄力数字回退 fallback: ", SEG_FONT_PATH)
		_ras_font = null
...
func _draw_b2b_badge():
	...
	if _b2b_ras_mode:
		num = num.replace("1", "I")
	# 【rAS】逆位全旋：蓄力数字用 DS Crystal 字体（cristal.ttf），其余用 fallback
	var font: Font = _get_ras_font() if _b2b_ras_mode else ThemeDB.fallback_font
	if font == null:
		font = ThemeDB.fallback_font
	# DS Crystal 字形高 ≈ 1.153× em（与 seg 数字一致），换算字号保持视觉高度
	var fs_ratio: float = 1.153 if _b2b_ras_mode else 1.0
	var fs := maxi(6, roundi(_b2b_badge_cell * 0.8 * _get_b2b_scale() * num_mult / fs_ratio))
