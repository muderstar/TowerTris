...
# ====== 【AS】全旋倒计时行（TETR.IO wound_timer 同款渲染） ======
## TETR.IO wound-tile 噪点纹理（整行背景瓦片）
var WOUND_TILE: Texture2D = preload("res://Assets/allspin/wound-tile.png")
## 行背景瓦片基色（TETR.IO 初始染色 #780013 暗红）
const WOUND_TILE_BASE_COLOR: Color = Color(0x78 / 255.0, 0.0, 0x13 / 255.0)
## DS Crystal 图集（4x 超采样 + 笔画纯白化，避免 Godot BMFont 导入损坏 + 小尺寸发糊）
var SEG_TEX: Texture2D = preload("res://Assets/allspin/seg-white-4x.png")
## seg.fnt 字形表（4x 图集坐标，原 atlas 坐标 ×4）：0-9
const SEG_GLYPHS: Dictionary = {
	"0": Rect2(0, 0, 244, 332), "1": Rect2(484, 664, 164, 328), "2": Rect2(244, 0, 244, 332),
	"3": Rect2(488, 0, 248, 332), "4": Rect2(736, 0, 244, 332), "5": Rect2(0, 332, 252, 332),
	"6": Rect2(252, 332, 244, 332), "7": Rect2(496, 332, 244, 332), "8": Rect2(740, 332, 244, 332),
	"9": Rect2(0, 664, 244, 332),
}
## 字形盒高对应的设计字号（4x 图集 = 288pt）
const SEG_DESIGN_FONT_SIZE: float = 288.0
## DS Crystal TrueType 字体（与 tetrio seg.fnt 同源，渲染数字 1 时显示为字符 I）
const SEG_FONT_PATH: String = "res://Assets/allspin/cristal.ttf"
## 懒加载的 DS Crystal 字体；加载失败时回退到图集渲染
var _seg_font: FontFile = null
var _seg_font_load_failed: bool = false
## 倒计时行动画累计时间（用于描边闪烁/行呼吸）
var _cd_anim_time: float = 0.0
## 进行中的数字动画：{row_y: {"kind": "spawn"/"good", "t": 0.0}}
var _cd_anims: Dictionary = {}
...
# 皮肤
var skin: MinoSkin = null              # 当前皮肤（null = 使用纯色）
...
var invisible: int = 0                       # 0=关闭, 1=开启（放置的方块隐藏，垃圾行正常显示）
...
func _ready():
	...
	# 【AS】倒计时 DS Crystal 图集需要 mipmap 线性采样（缩小渲染清晰）；整层统一设置
...
func _process(delta):
	...
	# 【AS】倒计时行动画驱动：有倒计时行或动画进行中 → 逐帧重绘（描边闪烁/行呼吸/数字弹出）
	var has_cd: bool = garbage_line_controller != null and not garbage_line_controller.countdown_rows.is_empty()
	if has_cd or not _cd_anims.is_empty():
		_cd_anim_time += delta
		for key in _cd_anims.keys():
			var a: Dictionary = _cd_anims[key]
			a["t"] = float(a["t"]) + delta
...
func _is_occupied_by_current_piece(board_x: int, board_y: int) -> bool:
	...
	var current_piece = piece_controller.current_piece
	var current_pos = piece_controller.current_position
...
func _draw_cell_rect(x: int, y: int, cell_rect: Rect2, cell_color: Color, alpha: float = 1.0):
	...
	# 【AS】全旋倒计时数字由 _draw_wound_rows 整行渲染（TETR.IO wound_timer 同款），此处无需绘制

## ========== 【AS】全旋倒计时行渲染（TETR.IO wound_timer 同款） ==========

## 计算倒计时行当前颜色（蓝→红渐变，TETR.IO 同款）与动画速度
## t = clamp01((1 - amt/15)^2)：新行 amt 大 → t 小（蓝），越接近归零 → t 大（红，警示）
func _wound_tint(amt: int) -> Dictionary:
	var tt: float = clampf(pow(1.0 - float(amt) / 15.0, 2.0), 0.0, 1.0)
	var r: float = 255.0 * tt
	var b: float = 83.0 + (130.0 * tt * 2.0 if tt < 0.5 else 130.0 * (1.0 - tt) * 2.0)
	return {
		"color": Color(r / 255.0, 0.0, b / 255.0),
		"speed": 0.1 + 0.9 * pow(tt, 1.5),
	}

## 触发某行数字动画（由 garbage_line_controller 调用）
func countdown_anim(row_y: int, kind: String) -> void:
	if kind == "spawn":
		_cd_anims[row_y] = {"kind": "spawn", "t": 0.0, "dur": 0.5}
	elif kind == "good":
		_cd_anims[row_y] = {"kind": "good", "t": 0.0, "dur": 0.2}

## 绘制所有倒计时行：整行 wound-tile 瓦片背景 + 顶部七段数码数字
func _draw_wound_rows():
	if garbage_line_controller == null or garbage_line_controller.countdown_rows.is_empty():
		return
	for cd in garbage_line_controller.countdown_rows:
		var row_y: int = int(cd["row_y"])
		var digit_x: int = int(cd["x"])
		var amt: int = int(cd["value"])
		if row_y < 0 or row_y >= grid_height + above_visible_rows:
			continue
		var tint: Dictionary = _wound_tint(amt)
		var base: Color = tint["color"]
		var speed: float = tint["speed"]
		# 整行瓦片背景（每格随机旋转 0/90/180/270°，颜色随“呼吸”振荡）
		for col in range(grid_width):
			var cell_rect := Rect2(cell_to_world(col, row_y), Vector2(cell_size, cell_size))
			var phase: float = (float(absi(hash(Vector2i(col, row_y * 7 + digit_x))) % 628)) / 100.0
			var k: float = 0.75 - 0.25 * speed + sin(0.1 * _cd_anim_time + phase) * (0.25 + 0.25 * speed)
			var tile_tint: Color = base * k
			var angle: float = (col * 5 + row_y * 3 + digit_x * 7) % 4 * PI * 0.5
			var center: Vector2 = cell_rect.position + cell_rect.size * 0.5
			draw_set_transform(center, angle, Vector2.ONE)
			draw_texture_rect(WOUND_TILE, Rect2(-cell_rect.size * 0.5, cell_rect.size), false, tile_tint)
		draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)
		# 数字本体：DS Crystal 字形直接按图集绘制（超采样图集 + mipmap，任意尺寸清晰）
		var text := str(amt)
		var digit_center: Vector2 = cell_to_world(digit_x, row_y) + Vector2(cell_size, cell_size) * 0.5
		# 生成/减数动画缩放
		var scale := 1.0
		if _cd_anims.has(row_y):
			var a: Dictionary = _cd_anims[row_y]
			var p: float = clampf(float(a["t"]) / float(a["dur"]), 0.0, 1.0)
			if a["kind"] == "spawn":
				scale = lerpf(0.0, 2.0, p / 0.5) if p < 0.5 else lerpf(2.0, 1.0, (p - 0.5) / 0.5)
			else:
				scale = lerpf(1.0, 1.3, p / 0.5) if p < 0.5 else lerpf(1.3, 1.0, (p - 0.5) / 0.5)
		var outline_col: Color = base
		outline_col.a = 0.6 + 0.4 * sin(0.15 * _cd_anim_time)
		draw_set_transform(digit_center, 0.0, Vector2(scale, scale))
		_draw_seg_digits(text, float(cell_size), outline_col)
		draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)

## 按 TrueType 字体绘制 DS Crystal 数字（cristal.ttf）：
## 数字 1 渲染为字符 "I"；白填充 + 动态红蓝粗描边
## 字形高度 ≈ 格子 1.45 倍（TETR.IO 同款大字），两位数自动按宽度收缩防重叠
func _draw_seg_digits(text: String, cell: float, outline_col: Color) -> void:
	var font: FontFile = _get_seg_font()
	if font == null:
		_draw_seg_digits_atlas(text, cell, outline_col)
		return
	# 数字 1 → 字符 I（DS Crystal 风格的 1 形似竖线 I）
	text = text.replace("1", "I")
	# 期望字形盒高：格子 1.45 倍（TETR.IO 同款大字）
	var box_h: float = cell * 1.45
	# DS Crystal 字形高 ≈ 1.153 × em（seg.fnt：72pt 时字形高 83px），换算 em 字号
	var fs: int = maxi(1, int(round(box_h / 1.153)))
	var gap: float = cell * 0.1
	var measured: Vector2 = font.get_string_size(text, HORIZONTAL_ALIGNMENT_LEFT, -1, fs)
	if text.length() > 1:
		# 两位数：收缩到约 1.5 倍格子宽度内（避免跨列重叠）
		var max_w: float = cell * 1.5
		if measured.x > max_w:
			fs = maxi(1, int(fs * max_w / measured.x))
			measured = font.get_string_size(text, HORIZONTAL_ALIGNMENT_LEFT, -1, fs)
	# 以实际字形盒（ascent+descent）垂直居中
	var glyph_h: float = float(font.get_ascent(fs) + font.get_descent(fs))
	var top: Vector2 = Vector2(-measured.x * 0.5, -glyph_h * 0.5)  # 以变换原点（格子中心）居中
	var outline_w: int = maxi(1, int(round(fs * 0.06)))
	# 基线（draw_string 的 y 是基线，需要往上偏移 ascent 对齐盒顶）
	var baseline: Vector2 = Vector2(top.x, top.y + float(font.get_ascent(fs)))
	# 描边（红蓝渐变 + 闪烁）
	draw_string_outline(font, baseline, text, HORIZONTAL_ALIGNMENT_LEFT, -1, fs, outline_w, outline_col)
	# 白色填充
	draw_string(font, baseline, text, HORIZONTAL_ALIGNMENT_LEFT, -1, fs, Color.WHITE)

## 懒加载 DS Crystal 字体（优先使用 Godot 导入的 FontFile，导出后仍可用）；
## 若导入缺失再回退 load_dynamic_font 直接读原始 ttf；最终失败回退图集渲染
func _get_seg_font() -> FontFile:
	if _seg_font != null or _seg_font_load_failed:
		return _seg_font
	if ResourceLoader.exists(SEG_FONT_PATH):
		...
		if imported is FontFile:
			_seg_font = imported
			return _seg_font
	_seg_font = FontFile.new()
	if _seg_font.load_dynamic_font(SEG_FONT_PATH) != OK:
		push_warning("cristal.ttf 加载失败，回退到图集渲染: ", SEG_FONT_PATH)
		_seg_font = null
		_seg_font_load_failed = true
	return _seg_font

## 按图集绘制 DS Crystal 数字（seg-white-4x.png 回退方案）：白填充 + 动态红蓝粗描边（8 方向偏移）
func _draw_seg_digits_atlas(text: String, cell: float, outline_col: Color) -> void:
	var box_h: float = 332.0
	var box_w: float = 244.0
	var fs: float = cell * 1.45                                  # 字形盒高
	var scale: float = fs / SEG_DESIGN_FONT_SIZE
	var glyph_w: float = box_w * scale
	var gap: float = cell * 0.1
	if text.length() > 1:
		# 两位数：收缩到约 1.5 倍格子宽度内（避免跨列重叠）
		var per: float = (cell * 1.5 - gap * float(text.length())) / float(text.length())
		fs = minf(fs, per / (box_w / box_h))
		scale = fs / SEG_DESIGN_FONT_SIZE
		glyph_w = box_w * scale
	var total_w: float = glyph_w * float(text.length()) + gap * float(maxi(0, text.length() - 1))
	var top: Vector2 = Vector2(-total_w * 0.5, -fs * 0.5)        # 以变换原点（格子中心）居中
	var outline_w: float = maxf(1.0, fs * 0.06)
	# 描边（8 方向偏移，颜色红蓝渐变 + 闪烁）
	for i in range(text.length()):
		var ch: String = text[i]
		if not SEG_GLYPHS.has(ch):
			continue
		var r: Rect2 = SEG_GLYPHS[ch]
		var dest := Rect2(top.x + float(i) * (glyph_w + gap), top.y, glyph_w, fs)
		for off in [Vector2(-1, 0), Vector2(1, 0), Vector2(0, -1), Vector2(0, 1),
				Vector2(-1, -1), Vector2(1, -1), Vector2(-1, 1), Vector2(1, 1)]:
			draw_texture_rect_region(SEG_TEX, dest.grow_individual(off.x * outline_w, off.y * outline_w, off.x * outline_w, off.y * outline_w), r, outline_col)
	# 白色填充
	for i in range(text.length()):
		var ch: String = text[i]
		if not SEG_GLYPHS.has(ch):
		...
		var r: Rect2 = SEG_GLYPHS[ch]
		var dest := Rect2(top.x + float(i) * (glyph_w + gap), top.y, glyph_w, fs)
		draw_texture_rect_region(SEG_TEX, dest, r, Color.WHITE)
...
## 应用皮肤（设置当前皮肤并重绘）
func apply_skin(new_skin: MinoSkin = null):
	skin = new_skin
	queue_redraw()
...
func _get_highest_block_y() -> int:
	...
	var current_cells := {}
	if piece_controller:
		var piece = piece_controller.current_piece
		var pos = piece_controller.current_position
		if piece and pos:
...
func _draw():
	...
	# 2.5 【AS】全旋倒计时行（TETR.IO wound_timer 同款：瓦片背景 + 七段数码，覆盖在格子上层）
	_draw_wound_rows()
...
## 获取消行控制器的引用
func set_clear_line_controller(controller: PieceClearLine):
