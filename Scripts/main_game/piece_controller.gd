...
@export var bag_controller: PieceBagController  # Bag生成器节点
...
#     tower_controller._extra_data_deal() 会把 piece_controller.bot_mode 置 true。
#     "BotMode" 键来自 buff_chose_area.gd 的 tower_init_data（关卡 buff 配置）。
#  2) 其余3个 @export 参数（bot_target_pps / bot_native_action_interval /
#     bot_debug_log）直接在 piece_controller.gd 的 Inspector 里调整。
# ---------------------------------------------------------------------------
...
# 在 piece_controller.gd 的 Inspector 中调整（@export）。
...
func _auto_finding():
	...
	if not bag_controller:
		bag_controller = get_node_or_null("../PieceBagController")
		if not bag_controller:
			push_error("PieceController: 未找到PieceBagController节点！")
			return
	
	# 自动查找clear_line_controller（如果未设置）
	if not clear_line_controller:
		clear_line_controller = get_node_or_null("../PieceClearLine")
		if not clear_line_controller:
			push_error("PieceController: 未找到PieceClearLine节点！")
			return
	
	# 自动查找garbage_line_controller（如果未设置）
	if not garbage_line_controller:
		garbage_line_controller = get_node_or_null("../MinoGarbageLineController")
		if not garbage_line_controller:
			push_error("PieceController: 未找到MinoGarbageLineController节点！")
			return
	...
	if not tower_controller:
		...
		if not tower_controller:
			push_error("PieceController: 未找到TowerController节点！")
			return
