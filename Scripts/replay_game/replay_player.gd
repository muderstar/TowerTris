...
func _find_garbage_controller(node: Node) -> MinoGarbageLineController:
	if node is MinoGarbageLineController:
		return node
	for child in node.get_children():
...
func load_and_prepare(path: String) -> bool:
	...
	# 应用录制时的设置
	if piece_controller:
		piece_controller.move_das = replay_data.move_das
		piece_controller.move_arr = replay_data.move_arr
		piece_controller.softdrop_delay = replay_data.softdrop_delay
...
func play():
	...
	# 开启回放输入覆盖模式
	if piece_controller:
		piece_controller.set_replay_input_override(true)
		print("已开启回放输入覆盖模式")
...
func stop():
	...
	for action in _active_actions:
		if piece_controller:
			piece_controller.set_replay_input(action, false)
	_active_actions.clear()

	# 关闭回放输入覆盖
	if piece_controller:
		piece_controller.set_replay_input_override(false)
...
func _apply_input_event(event: Dictionary, event_time: float) -> void:
	...
	if piece_controller:
		piece_controller.set_replay_input(action, pressed)
		if pressed:
...
func _handle_replay_end():
	...
	if piece_controller and piece_controller.is_inside_tree():
		await get_tree().create_timer(0.3).timeout
		if piece_controller.is_inside_tree():
			# 检查游戏是否已经自然结束（game_timer被_game_over停止了）
			var game_timer = piece_controller.get_node_or_null("game_timer")
			if not game_timer or not game_timer.is_stopped():
				piece_controller.game_ended.emit()
			else:
...
func seek_to_time(target_time: float):
	...
	for action in last_press_state:
		if last_press_state[action] and piece_controller:
			piece_controller.set_replay_input(action, true)
			_active_actions[action] = true
