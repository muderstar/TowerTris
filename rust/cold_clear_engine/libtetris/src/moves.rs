...
pub fn find_moves(board: &Board, mut spawned: FallingPiece, mode: MovementMode) -> Vec<Placement> {
    ...
    if board.column_heights().iter().all(|&v| v < 16) {
        ...
        let starts = match mode {
            ...
            _ => zero_g_starts(spawned.kind.0, board.spawn_y),
        };
        ...
        for (mut place, mut inputs) in starts {
            ...
            lock_check(place, &mut locks, inputs.clone(), board.lockout_y);
    ...
    while let Some(placement) = check_queue.pop() {
        ...
        lock_check(position, &mut locks, moves, board.lockout_y);
    }
...
fn lock_check(
    ...
    if piece.cells().iter().all(|&(_, y)| y >= lockout_y as i32) {
        return;
    }
...
fn zero_g_starts(p: Piece, spawn_y: u32) -> Vec<(FallingPiece, InputList)> {
    use Piece::*;
    use PieceMovement::*;
    use RotationState::*;
    match p {
        O => vec![
            start(O, North, 0, &[Left, Left, Left, Left], 7, spawn_y),
            start(O, North, 1, &[Left, Left, Left], 5, spawn_y),
            start(O, North, 2, &[Left, Left], 3, spawn_y),
            start(O, North, 3, &[Left], 1, spawn_y),
            start(O, North, 4, &[], 0, spawn_y),
            start(O, North, 5, &[Right], 1, spawn_y),
            start(O, North, 6, &[Right, Right], 3, spawn_y),
            start(O, North, 7, &[Right, Right, Right], 5, spawn_y),
            start(O, North, 8, &[Right, Right, Right, Right], 7, spawn_y),
        ],
        I => vec![
            start(I, North, 1, &[Left, Left, Left], 5, spawn_y),
            start(I, North, 2, &[Left, Left], 3, spawn_y),
            start(I, North, 3, &[Left], 1, spawn_y),
            start(I, North, 4, &[], 0, spawn_y),
            start(I, North, 5, &[Right], 1, spawn_y),
            start(I, North, 6, &[Right, Right], 3, spawn_y),
            start(I, North, 7, &[Right, Right, Right], 5, spawn_y),
            start(I, West, 0, &[Left, Ccw, Left, Left, Left], 7, spawn_y),
            start(I, West, 1, &[Left, Ccw, Left, Left], 5, spawn_y),
            start(I, West, 2, &[Left, Ccw, Left], 3, spawn_y),
            start(I, West, 3, &[Left, Ccw], 2, spawn_y),
            start(I, West, 4, &[Ccw], 1, spawn_y),
            start(I, East, 4, &[Cw], 1, spawn_y),
            start(I, East, 5, &[Right, Cw], 2, spawn_y),
            start(I, East, 6, &[Right, Cw, Right], 3, spawn_y),
            start(I, East, 7, &[Right, Cw, Right, Right], 5, spawn_y),
            start(I, East, 8, &[Right, Cw, Right, Right, Right], 7, spawn_y),
            start(I, East, -1, &[Left, Cw, Left, Left, Left, Left], 9, spawn_y),
            start(I, East, 0, &[Left, Cw, Left, Left, Left], 7, spawn_y),
            start(I, East, 1, &[Left, Cw, Left, Left], 5, spawn_y),
            start(I, East, 2, &[Left, Cw, Left], 3, spawn_y),
            start(I, East, 3, &[Left, Cw], 2, spawn_y),
            start(I, West, 5, &[Right, Ccw], 2, spawn_y),
            start(I, West, 6, &[Right, Ccw, Right], 3, spawn_y),
            start(I, West, 7, &[Right, Ccw, Right, Right], 5, spawn_y),
            start(I, West, 8, &[Right, Ccw, Right, Right, Right], 7, spawn_y),
            start(I, West, 9, &[Right, Ccw, Right, Right, Right, Right], 9, spawn_y),
            start(I, South, 1, &[Left, Cw, Left, Cw, Left], 5, spawn_y),
            start(I, South, 2, &[Cw, Left, Cw, Left], 4, spawn_y),
            start(I, South, 3, &[Cw, Left, Cw], 3, spawn_y),
            start(I, South, 4, &[Cw, Cw], 3, spawn_y),
            start(I, South, 5, &[Cw, Right, Cw], 3, spawn_y),
            start(I, South, 6, &[Cw, Right, Cw, Right], 4, spawn_y),
            start(I, South, 7, &[Right, Cw, Right, Cw, Right], 5, spawn_y),
        ],
        _ => vec![
            start(p, North, 1, &[Left, Left, Left], 5, spawn_y),
            start(p, North, 2, &[Left, Left], 3, spawn_y),
            start(p, North, 3, &[Left], 1, spawn_y),
            start(p, North, 4, &[], 0, spawn_y),
            start(p, North, 5, &[Right], 1, spawn_y),
            start(p, North, 6, &[Right, Right], 3, spawn_y),
            start(p, North, 7, &[Right, Right, Right], 5, spawn_y),
            start(p, North, 8, &[Right, Right, Right, Right], 7, spawn_y),
            start(p, West, 1, &[Left, Ccw, Left, Left], 5, spawn_y),
            start(p, West, 2, &[Left, Ccw, Left], 3, spawn_y),
            start(p, West, 3, &[Left, Ccw], 2, spawn_y),
            start(p, West, 4, &[Ccw], 1, spawn_y),
            start(p, East, 4, &[Cw], 1, spawn_y),
            start(p, East, 5, &[Right, Cw], 2, spawn_y),
            start(p, East, 6, &[Right, Cw, Right], 3, spawn_y),
            start(p, East, 7, &[Right, Cw, Right, Right], 5, spawn_y),
            start(p, East, 8, &[Right, Cw, Right, Right, Right], 7, spawn_y),
            start(p, East, 0, &[Left, Cw, Left, Left, Left], 7, spawn_y),
            start(p, East, 1, &[Left, Cw, Left, Left], 5, spawn_y),
            start(p, East, 2, &[Left, Cw, Left], 3, spawn_y),
            start(p, East, 3, &[Left, Cw], 2, spawn_y),
            start(p, West, 5, &[Right, Ccw], 2, spawn_y),
            start(p, West, 6, &[Right, Ccw, Right], 3, spawn_y),
            start(p, West, 7, &[Right, Ccw, Right, Right], 5, spawn_y),
            start(p, West, 8, &[Right, Ccw, Right, Right, Right], 7, spawn_y),
            start(p, West, 9, &[Right, Ccw, Right, Right, Right, Right], 9, spawn_y),
            start(p, South, 1, &[Left, Cw, Left, Cw, Left], 5, spawn_y),
            start(p, South, 2, &[Cw, Left, Cw, Left], 4, spawn_y),
            start(p, South, 3, &[Cw, Left, Cw], 3, spawn_y),
            start(p, South, 4, &[Cw, Cw], 3, spawn_y),
            start(p, South, 5, &[Cw, Right, Cw], 3, spawn_y),
            start(p, South, 6, &[Cw, Right, Cw, Right], 4, spawn_y),
            start(p, South, 7, &[Right, Cw, Right, Cw, Right], 5, spawn_y),
            start(p, South, 8, &[Right, Cw, Right, Cw, Right, Right], 7, spawn_y),
        ],
    }
}
