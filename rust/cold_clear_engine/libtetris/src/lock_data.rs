...
impl PlacementKind {
    ...
    pub(crate) fn get(cleared: usize, tspin: TspinStatus) -> Self {
        // 非T方块在 allspin/full-spin 规则下可能带 spin 状态消 3/4 行（例如 I-spin triple/quad）。
        // 标准 T-Spin 无 3 行 mini 概念，故 (3, Mini) 视作 Tspin3；4 行消行无 spin 变体（无
        // "Tetris spin"），故 (4, Mini)/(4, Full) 视作 Clear4。这些组合此前触发 unreachable!()
        // 使 bot 崩溃（卡死）。此处保证函数全定义，避免 panic。
        match (cleared, tspin) {
            ...
            (3, TspinStatus::Mini) => PlacementKind::Tspin3,
            (3, _) => PlacementKind::Tspin3,
            (4, TspinStatus::None) => PlacementKind::Clear4,
            (4, _) => PlacementKind::Clear4,
