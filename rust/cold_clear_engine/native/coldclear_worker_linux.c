/*
 * coldclear_worker_linux.c
 *
 * Cross-platform ColdClear decision worker for the tetris-tower Godot project.
 * Compiles on BOTH Windows (MinGW) and Linux (GCC) via #ifdef _WIN32.
 *
 * Same line-based protocol over stdin/stdout; dynamically loads the prebuilt
 * ColdClear C ABI at runtime:
 *   Windows: cold_clear.dll     (LoadLibrary/GetProcAddress)
 *   Linux:   libcold_clear.so   (dlopen/dlsym)
 *
 ...
 *
 * Build:
 *   Linux:  gcc -O2 -o coldclear_worker_linux coldclear_worker_linux.c -ldl
 *   Windows: gcc -O2 -o coldclear_worker.exe coldclear_worker_linux.c
 *
...
int main(int argc, char **argv) {
	...
	/* Resolve the native ColdClear library relative to this executable's own
	 * directory. On Windows: cold_clear.dll. On Linux: libcold_clear.so. */
	char lib_path[1024];
#if defined(_WIN32)
	DWORD len = GetModuleFileNameA(NULL, lib_path, sizeof(lib_path) - 32);
	if (len == 0 || len >= sizeof(lib_path) - 32) {
		printf("ERR cannot resolve exe path\n"); fflush(stdout);
		return 1;
	}
	char *slash = strrchr(lib_path, '\\');
	if (slash != NULL) strcpy(slash + 1, "cold_clear.dll");
	else strcpy(lib_path, "cold_clear.dll");
#else
	ssize_t rlen = readlink("/proc/self/exe", lib_path, sizeof(lib_path) - 32);
	if (rlen <= 0 || rlen >= (ssize_t)(sizeof(lib_path) - 32)) {
		printf("ERR cannot resolve exe path\n"); fflush(stdout);
		return 1;
	}
	lib_path[rlen] = '\0';
	char *slash2 = strrchr(lib_path, '/');
	if (slash2 != NULL) strcpy(slash2 + 1, "libcold_clear.so");
	else strcpy(lib_path, "libcold_clear.so");
#endif

#if defined(_WIN32)
	HMODULE m = LoadLibraryA(lib_path);
	if (m == NULL) {
		printf("ERR LoadLibrary failed for %s (err=%lu)\n", lib_path, (unsigned long)GetLastError());
		fflush(stdout);
		return 1;
	}
#define CC_SYM(t, name) ((t)GetProcAddress(m, name))
#else
	void *m = dlopen(lib_path, RTLD_NOW | RTLD_LOCAL);
	if (m == NULL) {
		printf("ERR dlopen failed for %s (%s)\n", lib_path, dlerror());
		fflush(stdout);
		return 1;
	}
#define CC_SYM(t, name) ((t)dlsym(m, name))
#endif

	Fn_launch_with_board launch_with_board = CC_SYM(Fn_launch_with_board, "cc_launch_with_board_async");
	Fn_destroy destroy = CC_SYM(Fn_destroy, "cc_destroy_async");
	Fn_reset reset_bot = CC_SYM(Fn_reset, "cc_reset_async");
	Fn_add_next_piece add_next = CC_SYM(Fn_add_next_piece, "cc_add_next_piece_async");
	Fn_request_next_move request_next = CC_SYM(Fn_request_next_move, "cc_request_next_move");
	Fn_block_next_move block_next = CC_SYM(Fn_block_next_move, "cc_block_next_move");
	Fn_default_options def_opts = CC_SYM(Fn_default_options, "cc_default_options");
	Fn_default_weights def_weights = CC_SYM(Fn_default_weights, "cc_default_weights");
#undef CC_SYM
