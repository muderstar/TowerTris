...
typedef struct CCOptions {
    ...
    uint8_t spawn_y;   /* row new pieces spawn when spawn_rule == CC_ROW_CUSTOM (0 = default 19) */
    uint8_t lockout_y; /* lockout threshold (0 = default 20) */
    CCPcPriority pcloop;
    uint32_t min_nodes;
    uint32_t max_nodes;
    uint32_t threads;
...
typedef struct CCWeights {
    ...
    int32_t base_damage[5];      /* 基础伤害表（下标=消行数 0..4） */
    int32_t tspin_damage[4];     /* T-Spin 伤害表（下标=消行数 1..3） */
    int32_t allspin_damage[4];   /* All-Spin 伤害表（下标=消行数 1..3，默认同 T-Spin） */
    int32_t combo_damage[32];    /* 连击伤害表（下标=连击数） */
    int32_t b2b_bonus;           /* 连续 BTB 加成 */
    int32_t pc_damage;           /* Perfect Clear 附加伤害 */
    int32_t send_mult_attack;    /* 攻击倍率（buff，千分比 1000=1.0） */
    int32_t mult_defend;         /* 防御倍率（buff，千分比 1000=1.0） */
    int32_t damage_eval_mult;    /* 伤害→评估分 换算系数 */
    int32_t allspin_enabled;     /* allspin 规则模式：0=allmini(非T卡住→minispin)，1=allspin(非T卡住→fullspin)，其余保留 */
    int32_t kick_table[64];      /* 游戏 asc 踢墙表 [dx,dy,dx,dy,...]（空=用标准SRS） */
    int32_t kick_table_len;      /* 踢墙表有效 (dx,dy) 对数（0=用标准SRS） */
} CCWeights;
