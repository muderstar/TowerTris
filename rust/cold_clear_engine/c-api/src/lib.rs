...
cenum! {
    ...
    enum CCSpawnRule => SpawnRule {
        ...
        CC_ROW_CUSTOM => SpawnRule::RowCustom(0)
    }
...
struct CCOptions {
    ...
    /// Row at which new pieces spawn when spawn_rule == CC_ROW_CUSTOM (else 0).
    spawn_y: u8,
    /// Lockout threshold (piece cells all at y >= lockout_y => locked out). 0 = default (20).
    lockout_y: u8,
    pcloop: CCPcPriority,
    min_nodes: u32,
    max_nodes: u32,
...
struct CCWeights {
    ...
    // 实际游戏伤害模型参数（与 bot::evaluation::Standard 对应）
    game_damage_enabled: bool,
    base_damage: [i32; 5],
    tspin_damage: [i32; 4],
    allspin_damage: [i32; 4],
    combo_damage: [i32; 32],
    combo_formula: i32,
    b2b_bonus: i32,
    pc_damage: i32,
    send_mult_attack: i32,
    mult_defend: i32,
    damage_eval_mult: i32,
    attack_efficiency_weight: i32,
    allspin_enabled: i32,
...
fn convert_from_c_options(options: &CCOptions) -> cold_clear::Options {
    let spawn_rule = match options.spawn_rule {
        CCSpawnRule::CC_ROW_CUSTOM => SpawnRule::RowCustom(options.spawn_y),
        _ => options.spawn_rule.into(),
    };
    cold_clear::Options {
        ...
        spawn_rule,
        spawn_y: if options.spawn_y == 0 { 19 } else { options.spawn_y as u32 },
        lockout_y: if options.lockout_y == 0 { 20 } else { options.lockout_y as u32 },
...
fn convert_from_c_weights(weights: &CCWeights) -> cold_clear::evaluation::Standard {
    cold_clear::evaluation::Standard {
        ...
        allspin1: weights.allspin1,
        allspin2: weights.allspin2,
        allspin3: weights.allspin3,
        allspin3plus: weights.allspin3plus,
        ...
        game_damage_enabled: weights.game_damage_enabled,
        base_damage: weights.base_damage,
        tspin_damage: weights.tspin_damage,
        allspin_damage: weights.allspin_damage,
        combo_damage: weights.combo_damage,
        combo_formula: weights.combo_formula,
        b2b_bonus: weights.b2b_bonus,
        pc_damage: weights.pc_damage,
        send_mult_attack: weights.send_mult_attack,
        mult_defend: weights.mult_defend,
        damage_eval_mult: weights.damage_eval_mult,
...
unsafe extern "C" fn cc_launch_with_board_async(
    ...
    if options.lockout_y > 0 {
        board.lockout_y = options.lockout_y as u32;
    }
...
/// 把 CCWeights 中的 kick_table（[dx,dy,dx,dy,...]）应用到 board。
fn apply_kick_table(board: &mut Board, weights: &CCWeights) {
    let n = weights.kick_table_len as usize;
    if n == 0 {
        board.kick_table.clear();
        return;
    }
    let mut kicks: Vec<(i32, i32)> = Vec::with_capacity(n);
    for i in 0..n {
        kicks.push((weights.kick_table[i * 2], weights.kick_table[i * 2 + 1]));
    }
    board.kick_table = kicks;
}
...
fn convert_weights(w: cold_clear::evaluation::Standard) -> CCWeights {
    CCWeights {
        ...
        game_damage_enabled: w.game_damage_enabled,
        base_damage: w.base_damage,
        tspin_damage: w.tspin_damage,
        allspin_damage: w.allspin_damage,
        combo_damage: w.combo_damage,
        combo_formula: w.combo_formula,
        b2b_bonus: w.b2b_bonus,
        pc_damage: w.pc_damage,
        send_mult_attack: w.send_mult_attack,
        mult_defend: w.mult_defend,
        damage_eval_mult: w.damage_eval_mult,
        attack_efficiency_weight: w.attack_efficiency_weight,
        allspin_enabled: w.allspin_enabled,
